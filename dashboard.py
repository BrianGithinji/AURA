...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ── Simulate dynamic congestion tied to live vehicle count ────────────────────
    base_count = st.session_state.get("vehicle_count", 14) if "vehicle_count" in st.session_state else (
        st.session_state.history[-1] if "history" in st.session_state else 14
    )

    def dynamic_congestion(base, offset):
        val = base + offset + random.randint(-3, 3)
        if val < 10:  return "Low"
        if val < 25:  return "Medium"
        return "High"

    def eta_from_congestion(c, base_min):
        mult = {"Low": 1.0, "Medium": 1.6, "High": 2.5}[c]
        return f"{int(base_min * mult)} min"

    if "route_tick" not in st.session_state:
        st.session_state.route_tick = 0
    st.session_state.route_tick += 1

    CENTER = (-1.2921, 36.8219)

    route_defs = [
        {"name": "Route A — Uhuru Highway",  "coords": [(-1.2921,36.8219),(-1.2950,36.8150),(-1.2980,36.8080),(-1.3010,36.8010)], "offset":  5, "base_eta": 7},
        {"name": "Route B — Mombasa Road",   "coords": [(-1.2921,36.8219),(-1.3000,36.8300),(-1.3080,36.8380),(-1.3150,36.8450)], "offset": -5, "base_eta": 6},
        {"name": "Route C — Ngong Road",     "coords": [(-1.2921,36.8219),(-1.2860,36.8150),(-1.2800,36.8080),(-1.2740,36.8010)], "offset":  0, "base_eta": 8},
        {"name": "Route D — Thika Road",     "coords": [(-1.2921,36.8219),(-1.2800,36.8300),(-1.2680,36.8380),(-1.2560,36.8450)], "offset": -8, "base_eta": 7},
        {"name": "Route E — Waiyaki Way",    "coords": [(-1.2921,36.8219),(-1.2880,36.8350),(-1.2840,36.8480),(-1.2800,36.8600)], "offset": 10, "base_eta": 9},
    ]

    routes = []
    for r in route_defs:
        cong = dynamic_congestion(base_count, r["offset"])
        routes.append({
            "name":       r["name"],
            "coords":     r["coords"],
            "congestion": cong,
            "eta":        eta_from_congestion(cong, r["base_eta"]),
            "distance":   f"{r['base_eta'] * 0.55:.1f} km",
        })

    color_map  = {"Low": "#00ff88", "Medium": "#ff8c00", "High": "#ff3b5c"}
    low_routes = [r for r in routes if r["congestion"] == "Low"]
    best_route = min(low_routes, key=lambda r: int(r["eta"].split()[0])) if low_routes else \
                 min(routes, key=lambda r: int(r["eta"].split()[0]))
    ...
    with map_col:
        ...
        for route in routes:
            ...
            fig.add_trace(go.Scattermapbox(
                lat=lats, lon=lons,
                mode="lines+markers",
                line=dict(width=7 if is_best else 4, color=color),
                marker=dict(size=9 if is_best else 5, color=color),
                name=route["name"],
                hovertemplate=(
                    f"<b>{route['name']}</b><br>"
        ...
        fig.update_layout(
            mapbox=dict(style="carto-darkmatter", center=dict(lat=CENTER[0], lon=CENTER[1]), zoom=12),
            paper_bgcolor="rgba(0,0,0,0)",
            plot_bgcolor="rgba(0,0,0,0)",
            margin=dict(l=0, r=0, t=0, b=0),
            height=500,
            legend=dict(bgcolor="rgba(4,20,40,0.9)", bordercolor="#0d3a5c",
                        borderwidth=1, font=dict(color="#cce8ff", size=11)),
            font=dict(color="#cce8ff"),
        )
        st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False},
                        key=f"route_map_{st.session_state.route_tick}")

    with info_col:
        st.markdown('<div class="section-title">Route Status</div>', unsafe_allow_html=True)
        for route in sorted(routes, key=lambda r: int(r["eta"].split()[0])):
            is_best = route["name"] == best_route["name"]
            ...
            st.markdown(
                ...
                f'<div style="font-family:Orbitron,sans-serif;font-size:.68rem;color:#cce8ff;margin-bottom:6px">'
                f'{"RECOMMENDED  " if is_best else ""}{route["name"]}'
                f'</div>'
                f'<div style="display:flex;justify-content:space-between;align-items:center">'
                f'<span class="badge badge-{route["congestion"].lower()}">{route["congestion"]}</span>'
        ...
        for label, color in [("Low — Clear","#00ff88"),("Medium — Slow","#ff8c00"),("High — Blocked","#ff3b5c")]:
            st.markdown(
